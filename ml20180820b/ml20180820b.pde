...
private void swapFaces() {
  ...
  // Only 1 image in the test images
  for (int z=0; z<num; z++) {
    ...
    for (int i=0; i<boundary_image2.size(); i++) {
      Point pt = new Point((int)boundary_image2.get(i).x, 
        (int)boundary_image2.get(i).y);
      hull.add(pt);
    }
    Mat mask = Mat.zeros(im2.rows(), im2.cols(), im2.depth());
    MatOfPoint mp = new MatOfPoint();
    mp.fromList(hull);
    Imgproc.fillConvexPoly(mask, mp, Scalar.all(255));

    mp.fromList(boundary_image2);
    Rect r = Imgproc.boundingRect(mp);
    Point centre = new Point((r.tl().x + r.br().x)/2.0, 
      (r.tl().y + r.br().y)/2.0);
